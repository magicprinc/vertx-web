...
class OpenAPIRouterHandlerTest {
  ...
  void testCreateNoEBConfig() {
    Operation operationMock = mockOperation(new JsonObject());
  ...
  void testInvalidEBConfig(String cause, JsonObject model) {
    Operation operationMock = mockOperation(model);
  ...
  Operation mockOperation(JsonObject model) {
    Operation operationMock = mock(Operation.class);
    when(operationMock.getOperationId()).thenReturn("Foo");
    when(operationMock.getOpenAPIModel()).thenReturn(model);
